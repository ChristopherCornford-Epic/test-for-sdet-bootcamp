...
namespace SdetBootcampDay1.Exercises
{
    ...
    public class TakeHomeExercises
    {
         ...
         */
        [Test]
        public void Test_StockQuantityAfterPurchase()
        {
        ...
        [Test]
        public void Test_OrderExceedingStockQuantity()
        {
            OrderHandler orderHandler = new OrderHandler();

            var ExceedStockExecption = Assert.Throws<ArgumentException>(() => 
            { 
                orderHandler.OrderAndPay(OrderItem.Fortnite, 101);
            });

            Assert.That(ExceedStockExecption, Is.EqualTo("Insufficient stock for item Fortnite"));
            Assert.That(orderHandler.GetStockFor(OrderItem.Fortnite), Is.EqualTo(100));
        }
        /**
         * TODO: write a test that creates a new instance of the OrderHandler class
         ...
         */
        [Test]
        public void Test_AddUnknownItem()
        {
            OrderHandler orderHandler = new OrderHandler();

            var UnknownItemException = Assert.Throws<ArgumentException>(() => 
            {
                orderHandler.AddStock(OrderItem.DayOfTheTentacle, 1);
            });

            Assert.That(UnknownItemException, Is.EqualTo("Unknown item DayOfTheTentacle"));
            Assert.That(orderHandler.GetStockFor(OrderItem.DayOfTheTentacle), Is.EqualTo(0));
        }
